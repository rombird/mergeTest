...
public class NoticeDto {
    ...
    // 파일 업로드 관련
    private List<MultipartFile> noticeFileUpload;
    private List<NoticeFileDto> noticeFileDtoList;

    private int noticeFileAttached;

    public NoticeDto(Long id, String noticeWriter, String noticeTitle, int noticeHits, LocalDateTime noticeCreateTime) {
        this.id = id;
    ...
    public static NoticeDto toNoticeDto(NoticeEntity noticeEntity){
        ...
        noticeDto.setNoticeCreateTime(noticeEntity.getCreatedTime());
        noticeDto.setNoticeUpdateTime(noticeEntity.getUpdatedTime());

        // 첨부파일 유무 체크
        if (noticeEntity.getNoticeFileAttached() == 0) {
            noticeDto.setNoticeFileAttached(0);
        } else {
            // 첨부파일이 있으면
            noticeDto.setNoticeFileAttached(1);

            // 파일 정보 변환 (Entity -> DTO List)
            List<NoticeFileEntity> noticeFileEntities = noticeEntity.getNoticeFileEntityList();
            ...
            for (NoticeFileEntity noticefileEntity : noticeFileEntities) {
                ...
                noticeFileDtoList.add(NoticeFileDto.toNoticeFileDto(noticefileEntity));
            }
